...
// Собираем все ключи OpenRouter в один массив (до 5 ключей)
...
if (OPENROUTER_KEYS.length === 0) {
    throw new Error("КРИТИЧЕСКАЯ ОШИБКА: Не найден ни один ключ OpenRouter (OPENROUTER_KEY1..5)!");
}
    ...
    async generateResponse(networkId, prompt, settings, sendMessageCallback) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            ...
            try {
                ...
                // Проверка на "мусорный" ответ
                if (!content || content.trim().length < 10 || /[\u{FFFD}]/u.test(content)) {
                    throw new Error("API вернул некорректный (мусорный) или пустой ответ.");
                }
                ...
                if (errorMessage.includes('Insufficient credits')) {
                    ...
                    if (sendMessageCallback) await sendMessageCallback(`_(API ключ #${keyIndex + 1} исчерпан, автоматически пробую следующий...)_`);
                ...
                if (errorMessage.includes('can only afford')) {
                    ...
                    if (match && match[1]) {
                        ...
                        if (affordableTokens > 0) {
                            ...
                            this.currentKeyIndex = keyIndex; // Откатываем индекс, чтобы повторить с этим же ключом
